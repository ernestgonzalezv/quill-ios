...
struct NoteUseCaseTests {
    ...
    func createRejectsBlank() async {
        let create = CreateNote(repository: InMemoryNoteRepository(), dates: clock)
    ...
    func noOpEditDoesNotBumpTimestamp() async throws {
        ...
        let repository = InMemoryNoteRepository([existing])

        let result = try await UpdateNote(repository: repository, dates: clock)(
            id: existing.id,
    ...
    func deleteClearsContent() async throws {
        ...
        try await DeleteNote(repository: repository, dates: clock)(id: existing.id)
    ...
    func deleteIsIdempotent() async throws {
        let existing = Note.stub()
        let repository = InMemoryNoteRepository([existing])
        let delete = DeleteNote(repository: repository, dates: clock)
    ...
    func missingNoteThrows() async {
        let id = UUID()
        let update = UpdateNote(repository: InMemoryNoteRepository(), dates: clock)
    ...
    func loadOrdersAndFilters() async throws {
        ...
        let notes = try await LoadNotes(repository: InMemoryNoteRepository([recent, pinnedOld, older, deleted]))()
    ...
    func searchIsInsensitive() async throws {
        ...
        let load = LoadNotes(repository: InMemoryNoteRepository([cafe, Note.stub(title: "Other", body: "")]))
    ...
    func togglePin() async throws {
        ...
        let result = try await TogglePin(repository: repository, dates: clock)(id: existing.id)
    ...
    func loadNoteSkipsTombstones() async throws {
        ...
        let load = LoadNote(repository: InMemoryNoteRepository([tombstone]))
