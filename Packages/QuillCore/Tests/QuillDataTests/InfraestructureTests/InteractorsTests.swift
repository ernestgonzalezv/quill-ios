...
private actor ScriptedHTTPClient: ProtoInteractorHTTP {
...
private actor RecordingSleeper: ProtoSleeper {
...
@Suite("InteractorNotePersistence")
struct SwiftDataNoteRepositoryTests {
    ...
    private func makeRepository() throws -> InteractorNotePersistence {
        InteractorNotePersistence(modelContainer: try .quill(inMemory: true))
    }
...
struct SyncNotesTests {
    private actor StubRemote: ProtoRemoteNoteStore {
    ...
    private actor StubCursor: ProtoSyncCursorStore {
    ...
    func fullRound() async throws {
        ...
        let report = try await LogicSyncNotes(repository: repository, remote: remote, cursor: cursor)()
    ...
    func failedPullDoesNotAdvanceCursor() async {
        ...
        let sut = LogicSyncNotes(repository: InMemoryRepository(), remote: FailingRemote(), cursor: cursor)
    ...
    private actor FailingRemote: ProtoRemoteNoteStore {
        func pull(since date: Date?) async throws -> RemoteChangeSet {
    ...
    private actor InMemoryRepository: ProtoNoteRepository {
