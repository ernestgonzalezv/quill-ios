...
public final class NoteListViewModel {
    ...
    private let loadNotes: LoadNotes
    private let loadNote: LoadNote
    private let createNote: CreateNote
    private let deleteNote: DeleteNote
    private let togglePin: TogglePin
    private let synchronizer: (any NoteSynchronizing)?
    private let storeChanges: (any NoteRepositoryObserving)?
    ...
    public init(
        ...
        storeChanges: (any NoteRepositoryObserving)? = nil,
    ...
    public func createDraftNote() async -> Note? {
        do {
            // A placeholder title, because `CreateNote` rejects blank drafts —
            // the store should never hold a row the list cannot render.
            let note = try await createNote(NoteDraft(title: String(localized: L10n.notesUntitled)))
            await reload()
            return note
        } catch {
            state = .failed(message: error.localizedDescription)
